...
int main(void) {
    ...
    for(ll i   = 1 ; i<=54 ; i++ ){
        if(i>=1 && i<=7){
            ...
            if(i!=8) graph[i].push_back(i-1) ;
            if(i!=16) graph[i].push_back(i+1) ;
            if(i%2==0){
            ...
            if(i!=17) graph[i].push_back(i-1) ;
            if(i!=27) graph[i].push_back(i+1) ;
            if(i%2==1){
                graph[i].push_back(i+11) ;
                graph[i+11].push_back(i) ;
            }
        }
        else if(i>= 28 && i<= 38 ){
            if(i!=28) graph[i].push_back(i-1) ;
            if(i!=38) graph[i].push_back(i+1) ;
            if(i%2==1){
                graph[i].push_back(i+10) ;
                graph[i+10].push_back(i) ;
            }
            ...
            if(i!=39) graph[i].push_back(i-1) ;
            if(i!=47) graph[i].push_back(i+1) ;
            if(i%2==0){
                graph[i].push_back(i+8) ;
                graph[i+8].push_back(i) ;
            }
            ...
            if(i!=48) graph[i].push_back(i-1) ;
            if(i!=54) graph[i].push_back(i+1) ;
        }
    }
    ...
    while(cin>>a){
        ...
        for(ll i = 0 ; i<=5 ; i++){
            ...
            ll hate ;
            for(ll j = 0 ; j<l ; j++){
                if(visited[ graph[hand][j]]==-1){
                    cnt++ ;
                    hate = graph[hand][j] ;
                }
            }
            if(cnt==1){
                hisab.push_back(hate) ;
            }
        }
        ...
        for(ll i = 0  ; i<l2 ; i++){
            ll hand = hisab[i] ;
            ll l3 = graph[hand].size() ;
            for(ll k  = 0 ; k<l3 ; k++){
                if(amar_map.count( graph[hand][k] )){
                    ans = 1 ;
                    break;
                }
            }
