...
int main(void) {
    ...
    while(tests--){
        ...
        ll max_x = LONG_LONG_MIN ;
        ll max_y = LONG_LONG_MIN ;
        while(n--){
            ll x1 , y1 , x2 , y2 , x3 , y3 , x4 , y4 ;
            inll(x1) ; inll(y1) ; inll(x2) ; inll(y2) ; inll(x3) ; inll(y3) ;
            inll(x4) ; inll(y4) ;
            min_x = min( min_x , min( x1, min( x2 , min( x3 , x4) ) ) ) ;
            min_y = min( min_y , min( y1, min( y2 , min( y3 , y4) ) ) ) ;
            max_x = max( max_x , max( x1, max( x2 , max( x3 , x4) ) ) ) ;
            max_y = max( max_y , max( y1, max( y2 , max( y3 , y4) ) ) ) ;
        }
        outll( ( max_x - min_x ) * ( max_y  - min_y ) ) ;
