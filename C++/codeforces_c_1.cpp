...
ll fun(ll start , map< ll , vector<ll> >graph){
    ll ans = 0 ;
    queue<ll>lists  ;
    lists.push(start) ;
    visited[start] = 1;
    while(!lists.empty()){
        ...
        for(ll i  = 0  ; i< l ; i++){
            if(!visited.count(graph[hand][i] )){
                fl = 1 ;
                visited[  graph[hand][i] ] = 1 ;
                lists.push( graph[hand][i] ) ;
                ll achi = graph[hand][i] ;
                if( ara[achi][0] == 0 ){
                    ara[achi][1] = 0 ;
                    ara[achi][2] = max(ara[hand][2] , ara[achi][1]) ;
                }
                else{
                    if(ara[hand][0]==1){
                        ara[achi][1] = ara[hand][1]+1 ;
                        ara[achi][2] = max(ara[achi][1] , ara[hand][2]) ;
                    }
                    else{
                        ara[achi][1] = 1 ;
                        ara[achi][2] = max(ara[hand][2] , ara[achi][1]) ;
