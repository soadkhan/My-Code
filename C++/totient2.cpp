...
long long int se_si;
bitset<100000>bs;//boundary is the size ....... use in prime
...
void sephi(lld n){
        sieve(n);
        for(lld i =1;i<=n;i++){
                if(is_prime(i)) phi[i]=i-1;
                else phi[i]=i;
        }
        for(lld i=0;primes[i]<=n;i++)
                for(lld j=primes[i]*2;j<=n;j+=primes[i])
...
lld big_mod(lld a,lld b,lld n){
        if(b==0) return 1%n;
        lld x=big_mod(a,b/2,n);
        x=(x*x)%n;
        if(b%2==1) x=(x*a)%n;
        return x;
}
