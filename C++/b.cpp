...
int main(void) {
    ll tests ;
    while(1==scanf("%lld" , &tests)){
        map<string  , ll > lists ;
        map<string , ll>step_lists ;
        ll step = 0  ;
        rep1(tests , cases){
            string a ;
            cin>>a ;
            ...
            if(!lists.count(a)){
                step++ ;
                if(a[1] == '+'){
                   printf("Instruction %lld: add %c to %c\n" , cases , b[0] , b[2] ) ;
                }
                else if(a[1]=='-'){
                    printf("Instruction %lld: subtract %c from %c\n" , cases , b[2] , b[0]) ;
                }
                else if(a[1]=='*'){
                    printf("Instruction %lld: multiply %c with %c\n" , cases , b[0] , b[2]) ;
                }
                else if(a[1]=='/'){
                    printf("Instruction %lld: divide %c by %c\n" , cases , b[0] , b[2]) ;
                }
                lists[a] = cases ;
                step_lists[a] = step ;
            }
